...
class WPNS_GitHub_Updater {
     ...
     * @var string
     */
    private $repo = 'JonakyDS/wp-nalda-sync';
     ...
     * @var string
     */
    private $cache_key = 'wpns_github_update_check';
    ...
    public function __construct() {
        $this->slug     = 'wp-nalda-sync';
        $this->basename = WPNS_PLUGIN_BASENAME;
        $this->version  = WPNS_VERSION;
    ...
    private function get_download_url( $release ) {
        ...
        if ( ! empty( $release->assets ) ) {
            foreach ( $release->assets as $asset ) {
                if ( preg_match( '/wp-nalda-sync.*\.zip$/i', $asset->name ) ) {
    ...
    public function plugin_info( $result, $action, $args ) {
        ...
        return (object) array(
            ...
            'short_description' => 'Automatically generates product CSV feeds from WooCommerce and uploads them to SFTP servers.',
            'sections'          => array(
                'description'  => 'WP Nalda Sync automatically generates product CSV feeds from WooCommerce and uploads them to SFTP servers for marketplace integration.',
    ...
    public function fix_source_dir( $source, $remote_source, $upgrader, $args ) {
        ...
        if ( strpos( $source_base, $this->slug ) === false ) {
            ...
            return new WP_Error(
                'rename_failed',
                __( 'Unable to rename the update folder.', 'wp-nalda-sync' )
            );
        }
    ...
    public function plugin_row_meta( $links, $file ) {
        if ( $this->basename === $file ) {
            $links[] = sprintf(
                ...
                __( 'View on GitHub', 'wp-nalda-sync' )
            );
        }
